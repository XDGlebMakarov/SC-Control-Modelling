...
x(:,1) = x_START;
FI = zeros(12,N);
FI(:,1) = firstints(x_START(1:3),x_START(4:6),params.mu);
%{
1 - h
2,3,4 - c(x,y,z)
5,6,7 - f(x,y,z)
...
tic()
for i=2:N
    x(:,i) = RKstep(x(:,i-1),t(i),dt,params);
    FI(:,i) = firstints(x(1:3,i),x(4:6,i),params.mu);
end
toc()
...
sgtitle('Passive orbital move - Complex, FI')
...
sgtitle('Passive orbital move - Complex, FI & Trajectory')
